...
# 从指定阶段开始（断点续跑）
python -m pkg_deploy.k8s_cluster.module_main install --stage 3

# 预览安装计划（不执行）
python -m pkg_deploy.k8s_cluster.module_main install --dry-run
```
...
```bash
# 完整卸载（需要确认）
python -m pkg_deploy.k8s_cluster.module_main uninstall

# 强制卸载（跳过确认）
python -m pkg_deploy.k8s_cluster.module_main uninstall --force
```
...
```bash
python -m pkg_deploy.k8s_cluster.module_main backup
```
...
```bash
# 单独执行指定阶段（0-7）
python -m pkg_deploy.k8s_cluster.module_main stage 0    # 仅环境预检
python -m pkg_deploy.k8s_cluster.module_main stage 1    # 仅系统初始化
python -m pkg_deploy.k8s_cluster.module_main stage 2    # 仅安装 containerd
python -m pkg_deploy.k8s_cluster.module_main stage 3    # 仅安装 K8s 组件
python -m pkg_deploy.k8s_cluster.module_main stage 4    # 仅 Master 初始化
python -m pkg_deploy.k8s_cluster.module_main stage 5    # 仅 Node 加入
python -m pkg_deploy.k8s_cluster.module_main stage 6    # 仅部署 CNI
python -m pkg_deploy.k8s_cluster.module_main stage 7    # 仅健康校验
```
...
python -m pkg_deploy.k8s_cluster.module_main uninstall-stage 1    # 恢复系统初始化
python -m pkg_deploy.k8s_cluster.module_main uninstall-stage 2    # 卸载 containerd
python -m pkg_deploy.k8s_cluster.module_main uninstall-stage 3    # 卸载 K8s 组件
python -m pkg_deploy.k8s_cluster.module_main uninstall-stage 4    # kubeadm reset Master
python -m pkg_deploy.k8s_cluster.module_main uninstall-stage 5    # 移除 Worker 节点
python -m pkg_deploy.k8s_cluster.module_main uninstall-stage 6    # 移除 Calico CNI

# 强制回滚（跳过确认）
python -m pkg_deploy.k8s_cluster.module_main uninstall-stage 4 --force
```
...
python -m pkg_deploy.k8s_cluster.module_main uninstall-stage 4

# 3. 修复问题后，从 Stage 4 继续
python -m pkg_deploy.k8s_cluster.module_main install --stage 4
```

### 升级/回滚（预留）

```bash
python -m pkg_deploy.k8s_cluster.module_main upgrade
python -m pkg_deploy.k8s_cluster.module_main rollback
```
...
system_init:
  ...
  kernel_pkg_deploy: { required: [overlay, br_netfilter] }
  sysctl_params:
...
# 然后从 Stage 5 恢复
python -m pkg_deploy.k8s_cluster.module_main install --stage 5
```
