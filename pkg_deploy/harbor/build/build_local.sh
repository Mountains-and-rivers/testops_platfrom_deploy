...
if docker images --format '{{.Repository}}:{{.Tag}}' | grep -q '^registry:2$'; then
    ...
    # _build_registry 含 git clone 多行 recipe，用 sed 删除中间所有 recipe 行
    find "${BUILD_DIR}" -name 'Makefile' -exec sed -i \
...
# valkey 和 db 同样用本地 tar 兜底（防止 _build 从网络下载失败）
if docker images --format '{{.Repository}}:{{.Tag}}' | grep -q '^valkey/valkey:9-alpine$' && ! docker images --format '{{.Repository}}:{{.Tag}}' | grep -q "^goharbor/harbor-valkey:v${HARBOR_VER}$"; then
    info "  valkey 使用本地 tar，跳过 _build_valkey"
    docker tag valkey/valkey:9-alpine "goharbor/harbor-valkey:v${HARBOR_VER}"
fi
if docker images --format '{{.Repository}}:{{.Tag}}' | grep -q '^postgres:15-alpine$' && ! docker images --format '{{.Repository}}:{{.Tag}}' | grep -q "^goharbor/harbor-db:v${HARBOR_VER}$"; then
    info "  postgres 使用本地 tar，跳过 _build_db"
    docker tag postgres:15-alpine "goharbor/harbor-db:v${HARBOR_VER}"
fi
