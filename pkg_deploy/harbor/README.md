...
浏览器打开 `https://192.168.0.102`，登录 `admin / Harbor12345`，忽略证书警告。

Projects → New Project → 名称填 `testops`，勾选 Public → OK。

### 2. 配置 Docker（所有客户端机器）

自签名证书需要配置 `insecure-registries`（**Harbor 服务器本机和其他机器都需要**）：

```bash
# 如果已有 daemon.json，用 jq 合并；否则直接创建
if [ -f /etc/docker/daemon.json ]; then
    # 合并现有配置 + insecure-registries
    python3 -c "
import json
with open('/etc/docker/daemon.json') as f:
    cfg = json.load(f)
cfg['insecure-registries'] = ['192.168.0.102']
