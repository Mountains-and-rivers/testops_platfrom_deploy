#!/usr/bin/env python3
"""
zentao_cli — 禅道开源版企业级 TestOps 自动化部署 CLI
"""
...
def destroy(delete_pvc, force):
    """销毁 K8s 中禅道全部资源"""
    from pkg_deploy.zentao.workflow.destroy import destroy_zentao
...
def upgrade(version):
    """滚动升级禅道版本"""
    from pkg_deploy.zentao.workflow.upgrade import upgrade_zentao
...
def plugin_install(plugin_name):
    """在线安装插件"""
    from pkg_deploy.zentao.plugin_mgr.plugin_operate import install_plugin_online
...
def plugin_install_offline(zip_path):
    ...
    from pkg_deploy.zentao.plugin_mgr.plugin_operate import install_plugin_offline
    install_plugin_offline(zip_path)

@plugin.command(name="list")
def plugin_list():
    """列出已安装插件"""
    from pkg_deploy.zentao.plugin_mgr.plugin_operate import list_plugins
    list_plugins()
...
def full_deploy(version, skip_build):
    ...
    from workflows.full_zentao_cli import full_deploy
